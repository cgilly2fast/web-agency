...
export const tenantAdminFieldAccess: FieldAccess = ({ req: { user }, doc }) => {
  ...
  if (!user || !user.tenantRole || !user.tenantRole.includes('admin')) return false

  const docTenant = typeof doc?.tenant === 'string' ? doc?.tenant : doc?.tenant.id
  const userTenant = typeof user.tenant === 'string' ? user.tenant : user.tenant?.id

  return docTenant === userTenant
}
